...
1. 한국어, **3단락**, 전체 **600~900자** (풍부하게 — 실질 지시가 촘촘해야 한다)
...
### 규칙
- 선수 지명 지시마다 **"뭐가 일어나는지" 한 마디 필수**. 슈팅·크로스·컷백·스루패스·하프스페이스 침투·풀백 유인·수적 우위 중 하나 이상 나와야 함
...
### 2단락 — 어떻게 풀 것인가 (공격·수비 핵심 지시, 280~400자)
...
### 3단락 — 경기 중 체크포인트 (체력·변곡점, 170~280자)
...
- **서두에 포메이션 이름 박기**: "이 경기는 4-2-3-1로..." ❌ → "이 경기는 빠른 선제점으로 간다" (포메이션은 2단락 이후 필요할 때만)
- **골키퍼(GK) 체력 언급**: GK는 원래 풀 쿼터 뛰는 게 기본. "문성우가 4쿼터 풀로 뛰어 체력 부담" 같은 당연한 말 절대 금지. playerWorkload·positionChangers 에서 **role/slot 이 GK 인 선수는 체력 대상에서 제외**
