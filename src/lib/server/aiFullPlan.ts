...
**체크 A — 전술 장면 어휘 (각 단락 최소 1개, 전체 3개 이상)**:
아래 단어 풀에서 **1단락·2단락·3단락 각 최소 1개씩** 자연스럽게 포함. 없으면 지시가 추상으로 그친 것 → 재작성.
...
### 구조 (한국어 **정확히 3단락**, 전체 **900~1400자** — 쿼터 하나도 빠지지 않게 풍부하게)
...
**2단락 (450~700자) — 쿼터별 공격·수비 핵심 지시 (실질 내용의 중심)**

🔴 **각 쿼터를 동등 비중으로 다뤄라**. 어느 쿼터 하나만 긴 문장 여러 개, 다른 쿼터는 한 줄로 스킵 — 이런 불균형 금지. 쿼터 4개면 네 쿼터 모두 **각각 100자 이상** 실질 지시.

- 각 쿼터마다 반드시:
  1) **해당 쿼터 포메이션 + 왜 이 쿼터는 이 구도인지 이유**
  2) **공격 루트** (선수 실명 2~3명 연쇄 — 누가 뭘 만들어 누구에게 연결)
  3) **수비 루트** (선수 실명 1~2명 + 구체 장면 단어)
- 포메이션 변화 쿼터는 **변화 이유 + 기대 효과** 한 쌍 필수 (위 '쿼터별 변화 설명' 섹션)
- 같은 포메이션이 반복돼도 **지시는 달라야 한다** — 체력 분배·상대 반응·경기 흐름 변화로 쿼터별 강조점 바꿔라 ("1쿼터는 A 중심 선제점, 3쿼터는 A 체력 빠지니까 B로 마무리 맡기자")
- 상대팀 이력 있으면 지난 맞대결 교훈을 지시로 / 최근 3경기 흐름 반영

**3단락 (250~400자) — 경기 중 체크포인트 + 교체 카드 + 마무리**
