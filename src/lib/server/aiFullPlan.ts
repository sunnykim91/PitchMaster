...
### 규칙
- **선수 지명 지시마다 "뭐가 일어나는지" 한 마디 더 붙여라**. 슈팅·크로스·컷백·스루패스·하프스페이스 침투·풀백 유인·수적 우위 중 하나는 반드시 나와야 한다
- 두세 선수를 **연결**해서 설명하면 더 좋다 ("A가 ~하면 → B가 그 공간으로 ~")
- "관리 모드", "안정적으로", "기세 유지" 같은 추상 수식 하나뿐인 지시 금지 — 뭘 **어떻게** 관리하는지 한 줄 붙여라 ("라인을 박스 밖 5m 로 내리고 상대 크로스는 측면 더블팀으로 차단" 같이)
...
### 구조 (한국어 **정확히 3단락**, 전체 600~900자 — 풍부하게 쓸 것)

**🔴 단락 수 엄수**: 반드시 3단락 (줄바꿈 2개로 구분). 4단락 이상 쓰면 실패. 체크포인트를 별도 단락으로 뽑지 말고 3단락 안에 묶어라.
...
**1단락 (150~220자) — 게임플랜 한 줄 + 근거 + 톤 설정**
- "이 경기는 X로 간다" 한 줄로 시작
- 근거 1~2개 (우리 폼·상대 성향·가용 자원 중 골라서)
- 마지막에 팀 분위기 짚는 한 마디 ("기세 놓치지 마", "이 경기 무조건 잡아야 해" 등)

**2단락 (280~400자) — 공격·수비 핵심 지시 (실질 내용의 중심)**
- 공격 루트: 누구를 중심으로 어디를 뚫을지. 커리어 스탯은 지명 근거로만
  - 주요 선수 2~3명 **이름 부르면서** 역할 구체 지시 ("민준아, 좌측 풀백 잡아놓고 건우한테 줘")
- 수비 루트: 어느 라인에서 끊을지, 뒷공간 어떻게 관리할지. 수비 핵심 1~2명 실명 지시
- 포메이션 변화가 있다면 변곡점 쿼터 **왜 + 뭘 기대하는지** (위 '쿼터별 변화 설명' 섹션 준수)
- 상대팀 이력 있으면 지난 맞대결 교훈을 지시로
- 최근 3경기 흐름 반영 ("연승 중이라 기세 유지")

**3단락 (170~280자) — 경기 중 체크포인트 + 마무리 한 마디**
