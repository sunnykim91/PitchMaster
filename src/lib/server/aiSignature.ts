...
4. **MOM, MVP, CS** 약어는 **글자 하나도 빠뜨리지 말 것**
   - MOM은 세 글자, MVP는 세 글자, CS는 두 글자
   - "MO", "MV" 등 불완전 약어 절대 금지 — 출력 전 한 글자씩 세어 확인할 것
5. **숫자와 문자 사이 공백 필수** ("5회 MOM" O, "5회MOM" X)
6. **이름 직접 언급 금지** — 카드에 이미 이름 있음. "그", "선수" 허용
7. **마침표·따옴표·이모지·괄호·말줄임표 금지**
8. **JSON/마크다운 없이 순수 텍스트 한 줄만**
...
## 마지막 체크리스트 (한 글자씩 세면서 확인)

출력 전 반드시 한 줄을 머릿속에 쓴 뒤, 아래 항목을 **순서대로** 점검:

1. 23자 이내? (공백 포함) — 넘으면 줄이기
2. **MOM이 M-O-M 세 글자로 완전한가?** (MO, MVP가 MV 아닌지)
3. 숫자와 한글 사이 공백 있는가? ("5 경기" 또는 "5회 MOM" 형식)
4. 금지 어휘 없는가? (기둥, 핵심, 에이스, 전설, 거머쥔 등)
5. 이름 언급 안 했는가?
6. 장면/감정이 떠오르는가? (추상 찬사 아님)
7. 선수가 자랑스러워할 한 줄인가?

**모두 통과해야** 출력. 하나라도 실패하면 다시 쓰세요.
