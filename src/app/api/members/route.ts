...
export async function DELETE(request: NextRequest) {
  ...
  // 강퇴 시 유니크 슬롯(등번호·주장/부주장)을 함께 비운다. 해당 unique 인덱스들은 status 무관이라
  // 슬롯을 쥔 채 BANNED 되면 그 번호/주장 자리가 영구 점유돼 다른 회원에게 재부여 불가.
  const { error } = await db
    .from("team_members")
    .update({ status: "BANNED", jersey_number: null, team_role: null })
    .eq("id", memberId)
    .eq("team_id", ctx.teamId);
