...
const FAQS: Array<{ q: string; a: string; cat: Cat }> = [
  ...
  { cat: "기능", q: "AI 라인업은 어떻게 동작하나요?", a: "참석자 명단·선호 포지션·과거 경기 기록·상대팀 이력을 분석해 포메이션과 쿼터별 출전을 자동 추천합니다. 명단만 넣으면 쿼터별 출전까지 자동으로 짜주는 방식은 아직 흔치 않아요." },
  ...
  { cat: "기능", q: "회비 OCR이 정확한가요?", a: "은행 앱 캡처를 올리면 입금자명·금액을 자동 인식해 명단과 매칭합니다. 휴회·부상 면제를 등록해 둔 회원은 매월 자동으로 면제 처리돼 미납으로 잡히지 않습니다." },
  ...
  { cat: "관리", q: "벌금은 자동으로 부과되나요?", a: "지각·불참·미투표 벌금 규칙을 한 번 설정해두면, 매번 묻지 않아도 자동으로 벌금이 부과돼 회비의 벌금 탭·납부 현황에서 한눈에 관리됩니다. (회비 잔액에서 바로 빠지는 게 아니라 별도 벌금으로 쌓여 납부받는 방식이에요.)" },
  ...
  { cat: "관리", q: "알림(푸시)이 안 와요. 어떻게 받나요?", a: "안드로이드(갤럭시 등)는 크롬·삼성 인터넷 어느 브라우저든 '홈 화면에 추가' 후 '알림 허용'을 하면 앱 없이도 푸시를 받을 수 있어요. 더 안정적으로 받고 싶으면 Google Play의 '피치마스터' 앱을 설치해도 됩니다(선택). 아이폰은 사파리에서 '홈 화면에 추가'를 한 뒤, 그 아이콘으로 처음 들어갈 때 뜨는 '알림 허용'을 눌러주세요. 그래도 안 오면 ① 설정 → 알림 토글이 켜져 있는지 ② 폰 자체 알림 권한이 켜져 있는지 ③ 카카오톡 안 브라우저(인앱)로 보고 있는 건 아닌지 확인하면 됩니다." },
  { cat: "관리", q: "운영진 권한 분리가 되나요?", a: "회장/총무/일반 팀원 3단계로 권한이 나뉘어 있습니다. 또 MVP 투표·기록 입력 같은 일부 기능은 팀 설정에서 운영진 전용으로 제한할 수 있어요." },
  { cat: "관리", q: "우리 팀 데이터는 안전한가요?", a: "출석·회비·경기 기록은 한국 서울 리전에 암호화되어 보관되고, 권한이 있는 운영진만 접근할 수 있습니다. 카카오 로그인만 사용해 별도 비밀번호는 저장하지 않습니다. 월별 결산은 이미지로 저장해 앱과 별개로도 기록을 남길 수 있습니다." },
  { cat: "기능", q: "팀 내부 자체전(홍백전)도 기록되나요?", a: "네, 자체전은 인원을 2~3팀(A·B·C)으로 나눠 편성하고 팀별 점수와 개인 골·어시 기록까지 남길 수 있습니다. 자체전 기록은 상대팀 전적과 분리되어 집계됩니다." },
  { cat: "기능", q: "경기 사진을 모아둘 수 있나요?", a: "네, 경기마다 사진을 올리면 게시판 '앨범' 탭에 자동으로 모여 팀원 누구나 지난 경기 사진을 한곳에서 볼 수 있습니다." },
];
