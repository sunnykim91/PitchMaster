...
const FAQS: Array<{ q: string; a: string; cat: Cat }> = [
  ...
  { cat: "관리", q: "알림(푸시)이 안 와요. 어떻게 받나요?", a: "알림은 휴대폰에 앱을 설치해야 옵니다. 안드로이드(갤럭시 등)는 Google Play에서 '피치마스터' 앱을 설치하세요 — 특히 기본 브라우저가 삼성 인터넷인 갤럭시에서는 홈 화면 추가만으로는 알림이 잘 안 오는 경우가 많은데, Play 스토어 앱으로 설치하면 확실히 해결됩니다. 아이폰은 사파리에서 '홈 화면에 추가'를 한 뒤, 그 아이콘으로 처음 들어갈 때 뜨는 '알림 허용'을 눌러주세요. 그래도 안 오면 ① 설정 → 알림 토글이 켜져 있는지 ② 폰 자체 알림 권한이 켜져 있는지 ③ 카카오톡 안 브라우저로 보고 있는 건 아닌지 확인하면 됩니다." },
