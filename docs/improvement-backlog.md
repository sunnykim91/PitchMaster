...
최종 업데이트: 2026-03-31 (8차)
현재 점수 추정: Designer 89 / UX 94 / Dev 95 / Marketing 84 / Business 82 (평균 88.8)
서비스 현황: 63팀 · 443명(가입 233 + 사전등록 210) · 23팀 활성(7일)
...
- [ ] **헤더 탭 → 홈 이동**: 모바일 헤더의 PitchMaster 로고/팀명이 클릭 불가. `<Link href="/dashboard">`로 감싸기
- [ ] **풀-투-리프레시 (당겨서 새로고침)**: 모든 데이터 페이지에서 아래로 당기면 새로고침되는 기본 모바일 제스처 없음
- [ ] **다크/라이트 모드 토글**: CSS 변수 기반이라 구조는 준비됨, 토글 UI + `prefers-color-scheme` 자동 감지 없음
- [ ] **오프라인 상태 감지 + UI 알림**: offline 페이지는 있지만 실시간 `navigator.onLine` 감지 없음. 연결 끊기면 API 실패 전까지 모름

### MEDIUM (불편하지만 치명적이지 않음)

- [ ] **이미지 로딩 placeholder**: 게시판 이미지, 팀 로고 등이 placeholder 없이 갑자기 나타남
- [ ] **페이지 전환 애니메이션**: 진입 애니메이션은 있지만 퇴장/라우트 전환 시 즉시 사라짐
- [ ] **폼 실시간 유효성 검사**: submit 시에만 검증, 입력 중 실시간 피드백 없음
- [ ] **키보드 열릴 때 레이아웃 대응**: 하단 탭바가 가상 키보드와 겹칠 수 있음
- [ ] **저장 안 한 변경사항 경고 확대**: 게시판/회칙만 `beforeunload` 있음, 경기 등록/설정 편집은 없음
- [ ] **새 콘텐츠 알림 배너**: 실시간 구독(투표/골)은 자동 갱신되지만, 게시판/대시보드는 "새 글이 있습니다" 배너 없음
- [ ] **뒤로가기 버튼 일관성**: 경기 상세만 있고, 다른 드릴다운 페이지(게시글 상세 등)에는 없음
