...
코드 기반 냉철한 분석 결과. 현재 규모(69팀 462명) 기준 시급성 판단 포함.

### CRITICAL (보안)

- [ ] **세션 HMAC 서명 추가**: pm_session 쿠키가 서명 없는 JSON → 쿠키 조작으로 다른 사용자 위장 가능. iron-session 또는 HMAC 서명 필요.
  - **현재 규모에서 필요?**: YES — 악의적 사용자 1명이면 전체 데이터 조작 가능. 다만 서비스 인지도가 낮아 공격 가능성은 낮음.
...
- [ ] **console.log 프로덕션 잔존**: `dues/route.ts:63,81`
  - **현재 규모에서 필요?**: YES — 즉시 제거 가능, 5분 작업.
