...
- [x] UX 사용성: 투표 로딩 스피너, 빈 상태 CTA, 필수 필드 표시, 투표 마감 안내, 장소 라벨, 포지션 카운터
- [x] 회원관리 모바일 더보기 메뉴 (휴면/제명을 ··· 메뉴로)
- [x] 입출금 필터 모바일 레이아웃 (검색+필터 별도 행 분리)
- [x] 회비 탭 전환 시 상단 스크롤
- [x] 게시판/회칙 작성 중 이탈 경고 (beforeunload)
- [x] 검색 input 자동완성 — 회원관리, 입출금, 투표관리 3곳 (datalist)
- [x] 회장 이임 모달 "제명" → "이임" 버튼 라벨 동적 변경
- [x] 회장 이임 후 세션 갱신 (페이지 리로드)
- [x] 자체전(Internal Match) 기능 전체 구현
  - 경기 유형 토글 (일반/자체전) + 통계 반영 체크박스
  - A팀/B팀 팀 편성 (랜덤 배정 + 수동 토글)
  - 골 기록 side(A/B) 구분 + A vs B 스코어보드
  - 전술판 A/B 팀 탭 분리 + 팀별 roster 필터링
  - 통계 stats_included 필터 + 대시보드 자체전 전적 제외
...
- [ ] PK/FK 골 기록 기능 (PK 득점, FK 득점 등 골 유형 분류)
