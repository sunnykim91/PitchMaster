...
### Phase 2 — 첫 진입 코치 마크 (HIGH, 별도 세션)
- **배경**: 가입 후 첫 화면이 빈 대시보드. 햄버거+탭바 두 네비 공존인데 50대 신규 회장은 메뉴 못 찾음.
- [ ] 1회성 코치 마크 5스텝 (햄버거→일정→회비→기록→설정 순)
- [ ] 또는 첫 매치 등록 CTA 카드 강조 (선택 필요)
- 참고: 50차 신규 추가 "코치 마크 / 첫 진입 튜토리얼" 항목과 통합

### Phase 2 — 회비 OCR 스크린샷 가이드 모달 (MEDIUM)
- **배경**: OCR 기능 이해 없이 그냥 "업로드" 시도 → 실패 후 이탈 추정.
- [ ] OCR 업로드 전 "어떤 스크린샷이 잘 인식되나요?" 가이드 모달 or 인라인 예시 이미지 추가.

### Phase 2 — 카카오 인앱 브라우저 안내 배너 (MEDIUM)
- **배경**: 카카오 인앱 브라우저에서 일부 기능 제한 (GA4 누락, 카메라 권한 등).
- [ ] 카카오 인앱 감지 시 "외부 브라우저로 열기" 배너 표시.
- [ ] 인앱 감지 조건: userAgent 내 KAKAOTALK 포함 여부.

### Phase 3 — 회원 벌크 CSV 등록 (HIGH)
- **배경**: 현재 한 명씩만 등록. 팀 창단 시 10~30명 일괄 등록 불편.
- [ ] CSV 템플릿 다운로드 + 업로드 파싱 UI.
- [ ] 검증: 이름 필수·중복·특수문자 차단 (`validateSafeName` 재사용).
- [ ] 참고: 기존 CLAUDE.md "실제 미구현 항목 2번" 항목.
...
### Phase 3 — 회원 펼침 모달 (LOW)
- **배경**: 회원 목록에서 회원 클릭 시 간단 정보 인라인 표시. 현재 별도 페이지 이동.
- [ ] 회원 클릭 → Bottom Sheet 또는 Dialog로 주요 정보 (출석률·회비·최근 경기) 노출.
- [ ] 기존 회원 상세 페이지는 유지 (딥링크 등 필요).
...
- [ ] 참고: CLAUDE.md "실제 미구현 항목 3번".

### Phase 4 (큰 작업) — 골 카드 쿼터별 그룹핑 (LOW)
- **배경**: 현재 골 카드는 시간순 나열. 쿼터별 그룹핑 시 전술 분석 용이.
- [ ] 쿼터 헤더 + 해당 쿼터 골 리스트 구조로 재편.
- [ ] null 쿼터(쿼터 모름) 별도 섹션.

### Phase 4 (큰 작업) — 자체전 PlayerPicker 통합 (LOW)
- **배경**: 자체전(INTERNAL) 경기에서 양팀 모두 PlayerPicker로 구성 가능하도록.
- [ ] 현재 자체전은 상대팀명 입력만 가능 → 팀 내 분리 구성 지원.
...
### 코치 마크 / 첫 진입 튜토리얼 (HIGH, 별도 세션)
- **배경**: 가입 후 첫 화면이 빈 대시보드. 햄버거+탭바 두 네비 공존인데 50대 신규 회장은 메뉴 못 찾음.
- [ ] 1회성 코치 마크 5스텝 (햄버거→일정→회비→기록→설정 순)
- [ ] 또는 첫 매치 등록 CTA 카드 강조
