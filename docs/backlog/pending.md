...
- [ ] `/guide` 인덱스 페이지 신설 — 발행된 가이드 목록 카드 (SEO + 내부 링크망 강화)
...
- [~] 스크린샷 경로 통일 (`/screenshot/` vs `/screenshots/`) — AppScreenSlider 내부 경로 단수 통일 완료(전수조사). 남은 건 `public/` 두 폴더 물리 일원화(후순위)
